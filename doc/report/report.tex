...
Domain Name System (DNS) is a naming system that allows the use of more easily memorizable names to access resources over the Internet instead of IP addresses. It is implemented as a hierarchy of name servers, where each name server maps a name to an IP address or otherwise points at other name servers that can be searched for the domain name. Thus DNS allows a computer to query a predefined name server for the IP corresponding to a domain name.
