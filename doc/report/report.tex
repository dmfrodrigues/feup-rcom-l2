...
To configure NAT in a commercial router, one should follow the aforementioned configuration commands. Generally speaking, setting up NAT requires one to:
\begin{enumerate*}
...
TCP uses a congestion control mechanism, which uses a congestion window to limit the bandwidth of simultaneous connections to avoid a severe decline in network performance.
The congestion window is managed in parallel with the receiver window, and the number of bytes in flight (bytes sent by the server but whose acknowledgement did not reach the server yet) is limited to the minimum between the receiver window size (\texttt{RWND}) and the congestion window size (\texttt{CWND}).
...
One popular method to implement this mechanism is Slow Start, where the server exponentially increases \texttt{CWND} until it reaches a certain threshold, and then enters a congestion management phase where \texttt{CWND} is linearly increased; this process is stopped when the sender detects packet loss (this is the congestion point), at which point it sets the threshold as half the congestion point, resets \texttt{CWND} to a very small value and starts over.

\texttt{RWND} is advertized in every \texttt{ACK} the receiver sends (displayed by Wireshark as \texttt{WIN}); on the other hand, \texttt{CWND} is maintained by the sender and is never advertized (the window size advertized by the server on \texttt{FTP-DATA} packets is its receiver window size, but since the server is not receiving much data nor does that number correspond to \texttt{CWND} it bears no importance for this matter).
Instead, one can try to observe some consequences of the variation of \texttt{CWND}.
The parameters that are usually expected to be affected by \texttt{CWND} are the number of bytes in flight perceived by the sender, and the throughput perceived by sender/receiver (after all the task of the congestion control mechanism is to restrict throughput to fairly distribute bandwidth over all connections).
...
In our logs \ref{listing:6-2-tux33-pic2.pcapng}, \ref{listing:6-5-tux32} we were unable to detect any influence of the congestion control mechanism, since in all connections the throughput instantly grew to ?, at which point it plateaued until the connection was closed or another download was started (see graphs \ref{fig:6-2-tux33-pic2-throughput}, \ref{fig:exp6_item5}).
We could not exactly track the reason behind this behaviour, although the following possibilities could have a contribution:
the server was not sending data fast enough;
it uses an algorithm that stabilizes almost instantly;
the server somehow already knows the bottleneck bandwidth of the physical path to the receiver.
