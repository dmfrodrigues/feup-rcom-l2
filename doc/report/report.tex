...
\usepackage[top=25mm,bottom=25mm,left=20mm,right=20mm]{geometry} % Margins
...
\usepackage[justification=centering,font=small,skip=0.2em]{caption}
...
Reports can be found in \ref{listing:6-2-tux33-pipe.download}, \ref{listing:6-2-tux33-pic2.download}, \ref{listing:6-5-tux32} and \ref{listing:6-5-tux33}.
...
% See issue reported in annex \ref{sec:configuration}.
...
% An \texttt{ACK} packet contains a sequence number, informing the server of the acknowledgement of all packets with sequence number less than that sequence number.
% A sliding window is used on the receiver; its size \texttt{RWND} is the number of octets, starting with the acknowledgement number, that the receiver is prepared to receive, and it is advertized in every \texttt{ACK} the receiver sends (displayed by Wireshark as \texttt{WIN}). If the server detects it is about to send a packet beyond what the receiver is prepared to receive, it assumes the packet immediately after the last acknowledged packet failed to transmit, and starts retransmission at that packet.

% It is nowadays very common for TCP stacks to implement selective acknowledgement (SACK), where the receiver can acknowledge a range of packets without acknowledging a previous packet, so the server only has to retransmit missing packets \cite{rfc2018}.
% In fact, both parties in logs \ref{listing:6-2-tux33-pipe.pcapng} and \ref{listing:6-2-tux33-pic2.pcapng} support SACK, as their \texttt{SYN} signals have \texttt{SACK\_PERMIT} set.

% TCP uses a congestion control mechanism consisting of a congestion window to limit the bandwidth of simultaneous connections to avoid a severe decline in network performance.
% The congestion window size \texttt{CWND} is managed in parallel with \texttt{RWND}, and the number of bytes in flight (bytes sent but not yet acknowledged) is limited to the minimum between \texttt{RWND} and \texttt{CWND}.
% % Since computers usually process data much faster than they can transmit it, and because \texttt{RWND} can be almost arbitrarily increased by taking up more RAM, \texttt{RWND} is not usually an issue.
% One popular method to implement this mechanism is Slow Start, where the server exponentially increases \texttt{CWND} until it reaches a certain threshold, and then enters a congestion management phase where \texttt{CWND} is linearly increased; this process is stopped when the sender detects packet loss (this is the congestion point), at which point it sets the threshold as half the congestion point, resets \texttt{CWND} to a typically small value and starts over.

% Unlike \texttt{RWND}, \texttt{CWND} is maintained by the sender and is never advertized.
% % \footnote{The window size advertized by the server on \texttt{FTP-DATA} packets is its \texttt{RWND}, which bears no importance for this matter.}.
% Instead, one can observe consequences of the variation of \texttt{CWND}.
% The parameters that are usually expected to be affected by \texttt{CWND} are the number of bytes in flight perceived by the sender, and the throughput perceived by sender/receiver.
% % \texttt{CWND} can also be estimated for instant $t$ from the number of packets received by the receiver in interval $[t-RTT/2, t+RTT/2)$ (which is linearly related with throughput, since during the data transfer phase the receiver almost only receives data packets, and these data packets all have the same size except for the last one).
% Since we do not have logs on sender traffic, we will focus on analyzing receiver traffic.

% In logs \ref{listing:6-2-tux33-pic2.pcapng}, \ref{listing:6-5-tux32} we were unable to detect any influence of the congestion control mechanism, since in all connections the throughput instantly grew to $\SI{100}{\mega\bit/\second}$, at which point it plateaued until the connection was closed or another download started (see graphs \ref{fig:6-2-tux33-pic2-throughput}, \ref{fig:exp6_item5}).
% We could not find the reason behind this behaviour, although the following facts could contribute:
% the sender was not sending data fast enough;
% it uses a congestion management algorithm that stabilizes almost instantly;
% the sender somehow already knows the bottleneck bandwidth of the physical path to the receiver.

% We also tested a simultaneous download in tux32 and tux33, logs \ref{listing:6-5-tux32} and \ref{listing:6-5-tux33}.
% We used a file with $\SI{88.1}{\mega\byte}$.
% First a download was started at tux32, and about 6 seconds later another download was started at tux33, \ref{fig:exp6_item5}.
% When the second download starts we see the total throughput is conserved but the throughput of each download varies considerably; theoretically, after a transitory regime all downloads should have similar throughputs, but in this case there was not enough time for stabilization.
% After the first download is complete, the second download takes up all bandwidth.
...
\subsection{Item 5}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:1-5-tux33-routes}, caption=\texttt{1-5-tux33-routes.txt}]{../../part2/exp1/1-5-tux33-routes.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:1-5-tux33-arp},    caption=\texttt{1-5-tux33-arp.txt}   ]{../../part2/exp1/1-5-tux33-arp.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:1-5-tux34-routes}, caption=\texttt{1-5-tux34-routes.txt}]{../../part2/exp1/1-5-tux34-routes.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:1-5-tux34-arp},    caption=\texttt{1-5-tux34-arp.txt}   ]{../../part2/exp1/1-5-tux34-arp.txt}
...
\subsection{Item 3}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:3-7-tux32.route}, caption=\texttt{3-7-tux32.route.txt}]{../../part2/exp3/3-4-tux32.route.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:3-7-tux33.route}, caption=\texttt{3-7-tux33.route.txt}]{../../part2/exp3/3-4-tux33.route.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small, frame=tbr, label={listing:3-7-tux34.route}, caption=\texttt{3-7-tux34.route.txt}]{../../part2/exp3/3-4-tux34.route.txt}

\setcounter{subsection}{6}
\subsection{Item 7}

\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:3-7-tux33.pcapng}, caption=\texttt{3-7-tux33.pcapng.txt}]{../../part2/exp3/3-7-tux33.pcapng.txt} \pagebreak
...
\setcounter{subsection}{2}
\subsection{Item 3}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:4-3-tux33.pcapng}, caption=\texttt{4-3-tux33.pcapng.txt}]{../../part2/exp4/4-3-tux33.pcapng.txt}

\setcounter{subsection}{3}
\subsection{Item 4}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-4c-tux32-noredirect-noroute.ping},   caption=\texttt{4-4c-tux32-noredirect-noroute.ping.txt}       ]{../../part2/exp4/4-4c-tux32-noredirect-noroute.ping.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:4-4c-tux32-noredirect-noroute.pcapng}, caption=\texttt{4-4c-tux32-noredirect-noroute.pcapng.txt}     ]{../../part2/exp4/4-4c-tux32-noredirect-noroute.pcapng.txt}
\end{landscape}

\begin{landscape}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-4e-tux32-noredirect-noroute.traceroute},  caption=\texttt{4-4e-tux32-noredirect-noroute.traceroute.txt} ]{../../part2/exp4/4-4e-tux32-noredirect-noroute.traceroute.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-4f-tux32-noredirect-yesroute.traceroute}, caption=\texttt{4-4f-tux32-noredirect-yesroute.traceroute.txt}]{../../part2/exp4/4-4f-tux32-noredirect-yesroute.traceroute.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-4g-tux32-yesredirect-noroute.ping},       caption=\texttt{4-4g-tux32-yesredirect-noroute.ping.txt}      ]{../../part2/exp4/4-4g-tux32-yesredirect-noroute.ping.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:4-4g-tux32-yesredirect-noroute.pcapng},     caption=\texttt{4-4g-tux32-yesredirect-noroute.pcapng.txt}    ]{../../part2/exp4/4-4g-tux32-yesredirect-noroute.pcapng.txt}
\end{landscape}

\begin{landscape}
\setcounter{subsection}{4}
\subsection{Item 5}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-5-tux33.ping},   caption=\texttt{4-5-tux33.ping.txt}  ]{../../part2/exp4/4-5-tux33.ping.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:4-5-tux33.pcapng}, caption=\texttt{4-5-tux33.pcapng.txt}]{../../part2/exp4/4-5-tux33.pcapng.txt}
\end{landscape}

\begin{landscape}
\setcounter{subsection}{6}
\subsection{Item 7}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:4-7-tux33.ping},   caption=\texttt{4-7-tux33.ping.txt}  ]{../../part2/exp4/4-7-tux33.ping.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:4-7-tux33.pcapng}, caption=\texttt{4-7-tux33.pcapng.txt}]{../../part2/exp4/4-7-tux33.pcapng.txt}
\end{landscape}
...
\setcounter{subsection}{2}
\subsection{Item 3}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\small     , frame=tbr, label={listing:5-3-tux33.ping},   caption=\texttt{5-3-tux33.ping.txt}  ]{../../part2/exp5/5-3-tux33.ping.txt}
\lstinputlisting[basicstyle=\linespread{0.85}\ttfamily\scriptsize, frame=tbr, label={listing:5-3-tux33.pcapng}, caption=\texttt{5-3-tux33.pcapng.txt}]{../../part2/exp5/5-3-tux33.pcapng.txt}
\end{landscape}
