...
Thus DNS allows a computer to query a predefined name server for the IP corresponding to a domain name.

To configure the DNS server at a host, you should add \texttt{nameserver <ip>} to file \texttt{/etc/resolv.conf}; this means the host will try to resolve a domain name by first asking the server at IP address \texttt{<ip>} for the IP corresponding to that domain name.
