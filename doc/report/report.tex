...
In the first phase of our TCP connection, a connection was established with a three-way handshake, a SYN command is sent to the server, the server then replied with a SYN-ACK and the client sent a ACK to the server.
...
This method sends ACK (acknowledgment message), when the data is received successfully before a timeout occurs, and a acknowledgment number indicating the next byte expected.
